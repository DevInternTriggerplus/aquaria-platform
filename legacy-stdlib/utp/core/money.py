...
RoundingMode = Literal[
    "NONE",
    # Legacy nearest/step modes (kept for existing venues and tests).
    "NEAREST_1", "NEAREST_5", "NEAREST_10", "UP_1", "DOWN_1",
    # Fix.md methods — direction is explicit, the step comes from the increment.
...
def apply_rounding(
    ...
    currency: str = "THB",
    *,
    increment_minor: int | None = None,
) -> int:
    """Apply the venue's configured rounding rule to a minor-unit amount.

    Rounding happens in exactly one place so the cart, receipt and tax invoice
    always reconcile (Fix.md §5, §17). All arithmetic is integer minor units — no
    floating point ever touches a total.

    ``mode``:

    * ``ROUND_UP`` — always round toward the next increment (Fix.md method 1).
    * ``ROUND_DOWN`` — always round toward the previous increment (method 2).
    * ``ROUND_HALF_UP`` — standard mathematical rounding: a remainder below half the
      increment rounds down, half or above rounds up (method 3).
    * Legacy ``NEAREST_1/5/10`` behave like ``ROUND_HALF_UP`` with a 1/5/10-major
      increment; ``UP_1``/``DOWN_1`` like ``ROUND_UP``/``ROUND_DOWN`` at 1 major.

    ``increment_minor`` is the rounding step in minor units. When omitted it defaults
    to one major unit of the currency (e.g. 100 satang = ฿1), which is the common
    Thai cash-rounding step. A currency with no minor units (JPY) already has a step
    of 1, so rounding to the nearest whole unit is a no-op there unless a larger
    increment is configured.
    """
    if mode in (None, "NONE"):
        return int(amount_minor)

    one_major = minor_units(currency)
    # Legacy step-named modes map onto the new (direction, increment) form.
    legacy_step = {"NEAREST_1": one_major, "NEAREST_5": 5 * one_major, "NEAREST_10": 10 * one_major,
                   "UP_1": one_major, "DOWN_1": one_major}
    if mode in legacy_step:
        step = legacy_step[mode]
        direction = "UP" if mode == "UP_1" else "DOWN" if mode == "DOWN_1" else "HALF_UP"
    elif mode == "ROUND_UP":
        step, direction = (increment_minor or one_major), "UP"
    elif mode == "ROUND_DOWN":
        step, direction = (increment_minor or one_major), "DOWN"
    elif mode == "ROUND_HALF_UP":
        step, direction = (increment_minor or one_major), "HALF_UP"
    else:
    ...
    # HALF_UP: round to the nearest step, ties (exactly half) go up.
...
class ChargeBreakdown:
    ...
    # Rounding snapshot (Fix.md §6): the amount before rounding, the method and the
    # increment used, so a completed order reproduces its own rounding exactly and a
    # later change to the venue's rounding setting never moves it (Fix.md §7, §16).
    pre_round_total_minor: int = 0
    rounding_mode: str = "NONE"
    rounding_increment_minor: int = 0
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "pre_round_total_minor": self.pre_round_total_minor,
            "rounding_mode": self.rounding_mode,
            "rounding_increment_minor": self.rounding_increment_minor,
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "service_charge_included": self.service_charge_included,
            "taxable_base_minor": self.taxable_base_minor,
            # Rounding snapshot — a completed order keeps how it was rounded (Fix.md §6/§7).
            "pre_round_total_minor": self.pre_round_total_minor,
            "rounding_mode": self.rounding_mode,
            "rounding_increment_minor": self.rounding_increment_minor,
            "rounding_adjustment_minor": self.rounding_adjustment_minor,
...
def compute_charges(
    *,
    ...
    rounding_increment_minor: int | None = None,
    currency: str = "THB",
) -> ChargeBreakdown:
    ...
    * **VAT EXCLUSIVE** adds VAT to (subtotal + any added service charge);
    ...
    grand_total = apply_rounding(
        pre_round_total, rounding_mode, currency, increment_minor=rounding_increment_minor
    )
    ...
    return ChargeBreakdown(
        ...
        pre_round_total_minor=pre_round_total,
        rounding_mode=rounding_mode if rounding_mode else "NONE",
        rounding_increment_minor=int(rounding_increment_minor or 0),
    )
