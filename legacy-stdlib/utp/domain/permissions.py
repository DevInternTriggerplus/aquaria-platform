...
PAGES: tuple[Page, ...] = (
    ...
    # The two dashboards are separate pages because they answer different
    # questions for different people: an executive wants business health, a duty
    # manager wants what needs attention right now. A venue supervisor is often
    # granted the operational view and denied the executive one.
    Page(
        "Operations Dashboard",
        "Operations Dashboard",
        "Insights",
        VIEW_ONLY,
        description="Today's arrivals, capacity, gate activity and device health (R71.1).",
    ),
    ...
    # ----------------------------------------------------------------- #
    # Settings pages (settings/reports spec §13)
    #
    # The spec's matrix marks some verbs "-", meaning the action does not
    # logically apply. A singleton configuration is viewed and edited; there is
    # no second VAT rate to add and no way to delete the concept of tax. Those
    # pages therefore declare ("VIEW", "EDIT") only, so ``page_key`` refuses to
    # build a key that could never be honoured and the matrix UI renders the
    # cell as not-applicable rather than as an unchecked box a user might try.
    # ----------------------------------------------------------------- #
    Page(
        "Organization",
        "Organization",
        "Configuration",
        delete_semantics="DEACTIVATE",
        description="Legal entity that owns venues. DELETE deactivates; historical records keep the reference.",
    ),
    Page(
        "Brand",
        "Brand",
        "Configuration",
        delete_semantics="DEACTIVATE",
        description="Customer-facing brand identity: name, logo, palette, tagline.",
    ),
    Page(
        "Operating Hours",
        "Operating Hours",
        "Business",
        ("VIEW", "EDIT"),
        description="Daily opening and closing times in the venue's own time zone.",
    ),
    Page(
        "Last Admission",
        "Last Admission",
        "Business",
        ("VIEW", "EDIT"),
        description="Latest time a guest may be admitted, and the sales cutoff derived from it (R6.4).",
    ),
    Page(
        "Booking Rules",
        "Booking Rules",
        "Ticket & Access",
        ("VIEW", "EDIT"),
        description="Party size limits, cutoffs, amendment and cancellation windows.",
    ),
    Page(
        "Advance Booking",
        "Advance Booking",
        "Ticket & Access",
        ("VIEW", "EDIT"),
        description="How far ahead a date becomes sellable, and when it opens.",
    ),
    Page(
        "QR Access Rules",
        "QR Access Rules",
        "Ticket & Access",
        ("VIEW", "EDIT"),
        description="Entry window, grace period, re-entry and maximum entry count (add_features §12).",
    ),
    Page(
        "Rounding",
        "Rounding",
        "Tax & Charges",
        ("VIEW", "EDIT"),
        description="Rounding rule applied once, after tax, so receipts reconcile with charges.",
    ),
    Page(
        "Price Display",
        "Price Display",
        "Tax & Charges",
        ("VIEW", "EDIT"),
        description="How prices are shown: inclusive labelling, symbol placement, decimal places.",
    ),
    Page(
        "Payment Providers",
        "Payment Providers",
        "Tax & Charges",
        delete_semantics="DEACTIVATE",
        description="Gateway/acquirer connections behind payment types. Credentials are never displayed.",
    ),
    Page(
        "Languages",
        "Languages",
        "Communications",
        delete_semantics="DEACTIVATE",
        description="Languages offered to customers and the default for this venue (R69).",
    ),
    Page(
        "Ticket Templates",
        "Ticket Templates",
        "Communications",
        delete_semantics="ARCHIVE",
        description="E-ticket and 80mm thermal ticket layouts.",
    ),
    Page(
        ...
        "Communications",
        delete_semantics="DEACTIVATE",
        description="Which transactional messages are sent, on which channel (R36, R37).",
    ),
    Page(
        "Terms & Conditions",
        "Terms & Conditions",
        "Communications",
        delete_semantics="ARCHIVE",
        description="Versioned terms and privacy notices. Published versions are archived, never edited (R12).",
    ),
    Page(
        "Gates",
        "Gates",
        "Configuration",
        delete_semantics="DEACTIVATE",
        description="Physical entrances. An access point is the scanning position at a gate.",
    ),
    Page(
        "Re-entry Rules",
        "Re-entry Rules",
        "Configuration",
        ("VIEW", "EDIT"),
        description="Whether a guest may leave and return, and within what window.",
    ),
    Page(
        ...
        "Configuration",
        ("VIEW", "EDIT"),
        description="Scanner behaviour: offline allowance, audible feedback, override prompts.",
    ),
    Page(
        ...
        "Seating",
        ("VIEW", "EDIT"),
        description="Hold duration, adjacency preference and release policy for reserved seating.",
    ),
    Page(
        "Permissions",
        "Permissions",
        "Administration",
        ("VIEW", "EDIT"),
        description="The permission registry itself: which pages and actions exist to be granted.",
    ),
    Page(
        "Login Security",
        "Login Security",
        "Administration",
        ("VIEW", "EDIT"),
        description="Lockout threshold, session timeouts, MFA and credential policy (R73).",
    ),
    Page(
        "POS Devices",
        "POS Devices",
        "Devices",
        delete_semantics="DEACTIVATE",
        description="Counter point-of-sale terminals.",
    ),
    Page(
        "Printers",
        "Printers",
        "Devices",
        delete_semantics="DEACTIVATE",
        description="Thermal and document printers bound to a counter or gate.",
    ),
    Page(
        "Gate Devices",
        "Gate Devices",
        "Devices",
        delete_semantics="DEACTIVATE",
        description="Registered scanners and turnstiles. DELETE deactivates the credential (R32.12).",
    ),
    Page(
        "Numbering",
        "Numbering",
        "System",
        ("VIEW", "EDIT"),
        description="Booking, receipt, invoice and credit-note number formats and sequences.",
    ),
    Page(
        "Integrations",
        "Integrations",
        "System",
        delete_semantics="DEACTIVATE",
        description="Outbound connections to partner, accounting and marketing systems.",
    ),
    Page(
        "API Configuration",
        "API Configuration",
        "System",
        delete_semantics="REVOKE",
        description="API clients and keys. DELETE revokes the credential; the record is retained.",
    ),
    Page(
        "Webhooks",
        "Webhooks",
        "System",
        delete_semantics="DEACTIVATE",
        description="Endpoints notified of platform events, and their signing secrets.",
    ),
    Page(
        ...
        "System",
        ("VIEW", "EDIT"),
        description="Low-level switches that should be changed only with support guidance.",
    ),
)
...
# --------------------------------------------------------------------------- #
# Settings information architecture (settings/reports spec §11, §12)
# --------------------------------------------------------------------------- #


@dataclass(frozen=True, slots=True)
class SettingsCategory:
    """One card on the Settings home, and one collapsible group in navigation.

    The category is presentation; the *pages* are the permission boundary. A
    category is shown only because at least one page inside it is viewable
    (§26, §71), so this structure never grants anything — it only decides what
    is worth drawing.
    """
    ...
    label: str
    description: str
    #: Stable slug the client maps to an inline SVG. Not an emoji and not a
    #: filename, so the backend never dictates presentation (§31).
    icon: str
    pages: tuple[str, ...]


#: Each page belongs to exactly one category. Overlap would double-count the
#: "3 of 7 configured" style summaries on the Settings home and make the
#: navigation ambiguous, so the spec's duplicated entries (QR rules appear under
#: both Booking & Ticketing and Access Control) are resolved to a single home.
SETTINGS_CATEGORIES: tuple[SettingsCategory, ...] = (
    SettingsCategory(
        "business",
        "Business",
        "Organization, brand, venue, opening hours and time zone.",
        "business",
        ("Organization", "Brand", "Venues", "Areas", "Operating Hours", "Last Admission", "Time Zone Settings"),
    ),
    SettingsCategory(
        "booking_ticketing",
        "Booking & Ticketing",
        "Ticket types, capacity, booking rules and ticket validity.",
        "ticket",
        (
            "Ticket Types",
            "Customer Segments",
    ...
    SettingsCategory(
        "pricing_tax",
        "Pricing & Tax",
        "VAT, service charge, currency, exchange rates and rounding.",
        "receipt",
        (
            "Currency Settings",
            "Exchange Rates",
    ...
    SettingsCategory(
        "payment",
        "Payments",
        "Customer payment options and the providers behind them.",
        "card",
        ("Payment Type", "Payment Providers"),
    ),
    SettingsCategory(
        "promotions",
        "Promotions",
        "Promotion rules, coupons, cash coupons, rewards and partner benefits.",
        "gift",
        ("Promotions", "Coupon Codes", "Cash Coupons", "Member Rewards", "Partner Benefits"),
    ),
    SettingsCategory(
        "customer_experience",
        "Customer Experience",
        "Languages, email and ticket templates, notifications and terms.",
        "globe",
        ("Languages", "Email Templates", "Ticket Templates", "Customer Notifications", "Terms & Conditions"),
    ),
    SettingsCategory(
        "access_control",
        "Access Control",
        "Gates, access points, re-entry and scanner behaviour.",
        "door",
        ("Gates", "Access Points", "Re-entry Rules", "Scanner Configuration"),
    ),
    SettingsCategory(
        "shows_seating",
        "Shows & Seating",
        "Show master data, schedule, seat types, zones and layouts.",
        "stage",
        ("Shows", "Show Schedule", "Seat Type", "Seat Zone", "Seat Layout", "Seat Reservation Rules"),
    ),
    SettingsCategory(
        "staff_security",
        "Staff & Security",
        "Staff accounts, roles, permissions, login security and audit.",
        "shield",
        ("Staff", "Roles", "Permissions", "Login Security", "Audit Logs"),
    ),
    SettingsCategory(
        "devices",
        "Devices",
        "Kiosks, counter terminals, printers, gate scanners and their health.",
        "device",
    ...
    SettingsCategory(
        "system",
        "System",
        "Numbering, integrations, API access, webhooks and advanced switches.",
        "system",
        ("Numbering", "Integrations", "API Configuration", "Webhooks", "Advanced Configuration"),
    ),
)

SETTINGS_CATEGORIES_BY_KEY: dict[str, SettingsCategory] = {c.key: c for c in SETTINGS_CATEGORIES}

#: Reverse lookup so a page can name its own category without scanning.
SETTINGS_CATEGORY_BY_PAGE: dict[str, str] = {
    page_name: category.key for category in SETTINGS_CATEGORIES for page_name in category.pages
}

#: Every page that appears somewhere in the Settings tree. Pages outside this set
#: (Bookings, Reports, the dashboards) are back-office *work*, not configuration,
#: and must not surface on the Settings home.
SETTINGS_PAGE_KEYS: frozenset[str] = frozenset(SETTINGS_CATEGORY_BY_PAGE)


def _validate_settings_tree() -> None:
    """Fail at import if the tree names a page that does not exist.

    A typo here would silently drop a category card and hide a settings page from
    every administrator, which is exactly the class of bug that is invisible in
    review and obvious in production.
    """
    seen: set[str] = set()
    for category in SETTINGS_CATEGORIES:
        for page_name in category.pages:
            if page_name not in PAGES_BY_KEY:
                raise ValueError(f"settings category {category.key!r} names unknown page {page_name!r}")
            if page_name in seen:
                raise ValueError(f"page {page_name!r} appears in more than one settings category")
            seen.add(page_name)


_validate_settings_tree()
...
ACTIONS: tuple[ActionPermission, ...] = (
    ...
    ActionPermission(
        ...
        "Insights",
        description=(
            "Separate from EXPORT: scheduling sends data out repeatedly and "
            "unattended, to a recipient chosen once (reports spec §38)."
        ),
    ),
    ActionPermission(
    ...
    # --- settings/reports spec §18: actions beyond CRUD --- #
    # CRUD is the base model, not the whole model. These are the operations the
    # spec singles out because holding EDIT on a page should not automatically
    # confer them: publishing makes a change customer-visible, approving is a
    # second pair of eyes, and resetting someone's access is an account takeover
    # if misused.
    ActionPermission(
        "RESET_ACCESS",
        "Reset staff access",
        ...
        requires_reason=True,
        description=(
            "Issue a fresh enrolment token and revoke live sessions. Separate from "
            "Staff.EDIT because it hands over the ability to sign in as that account."
        ),
    ),
    ActionPermission(
        "ASSIGN_ROLE",
        ...
        "Administration",
        description=(
            "Attach a role to a staff member at a scope. Never grants authority above "
            "the actor's own (R44.3)."
        ),
    ),
    ActionPermission(
        ...
        "Currency",
        revenue_affecting=True,
        description="Second authorization for a rate that will convert real money.",
    ),
    ActionPermission(
        "APPROVE_TAX_CHANGE",
        "Approve tax change",
        "Tax & Charges",
        revenue_affecting=True,
        description="Second authorization for a VAT or service-charge change.",
    ),
    ActionPermission(
        "MANAGE_LOGIN_SECURITY",
        "Manage login security",
        "Administration",
        description="Lockout thresholds, session timeouts, MFA and credential policy (R73).",
    ),
    ActionPermission(
        "MANAGE_INTEGRATION",
        ...
        "System",
        description=(
            "Integrations, API clients and webhook secrets. A leaked credential is a "
            "tenant-wide exposure, so this is not part of any page's EDIT."
        ),
    ),
)
...
def settings_tree(granted: set[str] | frozenset[str]) -> list[dict[str, object]]:
    """The Settings home and navigation, filtered to what ``granted`` may VIEW.

    Categories with no viewable page are omitted entirely rather than rendered
    empty (§26, §71): an administrator who cannot see a single device setting has
    no use for a Devices card that leads nowhere, and showing it leaks the shape
    of the configuration they are not trusted with.

    This is presentation only. Every page still enforces its own permission on
    the request that opens it, so a client that ignores this list gains nothing
    (§46, §75).
    """
    ...
    for category in SETTINGS_CATEGORIES:
        ...
        for page_name in category.pages:
            page = PAGES_BY_KEY[page_name]
            if f"{page.key}.VIEW" not in granted:
                continue
            pages.append(
                {
                    "page": page.key,
                    ...
                    "verbs": {verb: (verb in page.verbs) for verb in ALL_VERBS},
                    "can_view": True,
                    "can_add": f"{page.key}.ADD" in granted,
                    "can_edit": f"{page.key}.EDIT" in granted,
                    "can_delete": f"{page.key}.DELETE" in granted,
                    "delete_semantics": page.delete_semantics,
                    "protected": page.protected,
                }
            )
...
ROLE_TEMPLATES: tuple[RoleTemplate, ...] = (
    ...
    # A full-authority owner/administrator that signs in with a password alone. It
    # holds every page and every action, like the two admin roles, but is
    # deliberately NOT in HIGH_AUTHORITY_ROLE_CODES, so it is not forced through MFA
    # (which the demo UI cannot perform). Use it for a real operator who needs full
    # access without an authenticator app. The two MFA-gated admin roles above still
    # require MFA, so R73.2 is unchanged for them.
    RoleTemplate(
        code="OWNER",
        ...
        all_actions=True,
        mfa_required=False,
        description="Full authority without MFA — for a trusted operator/owner account.",
    ),
    RoleTemplate(
        code="VENUE_MANAGER",
        name="Venue Manager",
        ...
        grants={
            ...
            "Payment Type": _FULL,
            # --- settings pages (§13) --- #
            # A venue manager configures their own venue's operation. Anything
            # that reaches beyond one venue — the organization record, API
            # credentials, webhook secrets, advanced switches — is read-only or
            # absent, because a venue-scoped role should not be able to change
            # how other venues behave.
            "Organization": _RO,
            "Brand": _RO,
            "Venues": ("VIEW", "EDIT"),
            "Operating Hours": ("VIEW", "EDIT"),
            "Last Admission": ("VIEW", "EDIT"),
            "Booking Rules": ("VIEW", "EDIT"),
            "Advance Booking": ("VIEW", "EDIT"),
            "QR Access Rules": ("VIEW", "EDIT"),
            "Rounding": _RO,
            "Price Display": ("VIEW", "EDIT"),
            "Payment Providers": _RO,
            "Languages": _RW,
            "Ticket Templates": _RO,
            "Customer Notifications": _RW,
            "Terms & Conditions": _RO,
            "Gates": _RW,
            "Re-entry Rules": ("VIEW", "EDIT"),
            "Scanner Configuration": ("VIEW", "EDIT"),
            "Seat Reservation Rules": ("VIEW", "EDIT"),
            "Permissions": _RO,
            "Login Security": _RO,
            "POS Devices": _RO,
            "Printers": _RO,
    ...
    RoleTemplate(
        ...
        grants={
            ...
            "Seat Reservation": _RW,
            "Seat Layout": _RO,
            # A supervisor needs to *know* the rules they enforce at the door,
            # without being able to change them mid-shift.
            "Operating Hours": _RO,
            "Last Admission": _RO,
            "Gates": _RO,
            "Re-entry Rules": _RO,
            "Seat Reservation Rules": _RO,
        },
        permissions=(
            f"{ACTION_PREFIX}APPROVE",
            f"{ACTION_PREFIX}REFUND",
    ...
    RoleTemplate(
        ...
        grants={
            ...
            "Payment Type": _RW,
            "Cash Coupons": _FULL,
            # Finance owns how money is presented and numbered, and approves
            # rate and tax changes, but configures no operational behaviour.
            "Organization": _RO,
            "Brand": _RO,
            "Rounding": ("VIEW", "EDIT"),
            "Price Display": _RO,
            "Payment Providers": _RW,
            "Numbering": ("VIEW", "EDIT"),
        },
        permissions=(
            f"{ACTION_PREFIX}EXPORT",
            f"{ACTION_PREFIX}ISSUE_TAX_INVOICE",
            f"{ACTION_PREFIX}APPROVE_EXCHANGE_RATE",
            f"{ACTION_PREFIX}APPROVE_TAX_CHANGE",
            f"{ACTION_PREFIX}VIEW_COST",
            f"{ACTION_PREFIX}REFUND",
            f"{ACTION_PREFIX}MANAGE_TAX_SETTINGS",
            f"{ACTION_PREFIX}MANAGE_SERVICE_CHARGE",
    ...
    RoleTemplate(
        ...
        grants={
            ...
            "Show Schedule": _RO,
            "Reports": _RO,
            # Marketing owns everything the customer reads.
            "Brand": _RO,
            "Languages": _RW,
            "Ticket Templates": _RW,
            "Customer Notifications": _RW,
            "Terms & Conditions": _RO,
        },
        permissions=(
    ...
    RoleTemplate(
        ...
        name="Report Viewer",
        authority_level=20,
        grants={"Dashboard": _RO, "Operations Dashboard": _RO, "Reports": _RO},
        # EXPORT deliberately not granted: reading a figure on screen and taking a
        # copy of the underlying rows away are different privileges (R41.7). A
        # tenant that wants this role to download adds EXPORT explicitly.
        description="Read-only insight access. No PII, no cost, no export.",
    ),
    RoleTemplate(
        ...
        grants={
            "Dashboard": _RO,
            # Device and gate health live on the operational dashboard.
            "Operations Dashboard": _RO,
            "Devices": _FULL,
            "Kiosks": _FULL,
            "Access Points": _RW,
            ...
            "Audit Logs": _RO,
            "Settings": _RO,
            # The fleet and the plumbing behind it. Deliberately no money pages:
            # the people who can reach every terminal should not also be able to
            # change a price or a tax rate.
            "POS Devices": _FULL,
            "Printers": _FULL,
            "Gate Devices": _FULL,
            "Gates": _RW,
            "Scanner Configuration": ("VIEW", "EDIT"),
            "Integrations": _RW,
            "API Configuration": _RO,
            "Webhooks": _RW,
            "Advanced Configuration": _RO,
            "Numbering": _RO,
            "Login Security": _RO,
        },
        permissions=(f"{ACTION_PREFIX}REPRINT", f"{ACTION_PREFIX}MANAGE_INTEGRATION"),
...
__all__ = [
    ...
    "RoleTemplate",
    "SETTINGS_CATEGORIES",
    "SETTINGS_CATEGORIES_BY_KEY",
    "SETTINGS_CATEGORY_BY_PAGE",
    "SETTINGS_PAGE_KEYS",
    "SUPER_ADMIN_CODE",
    "SettingsCategory",
    "VIEW_ONLY",
